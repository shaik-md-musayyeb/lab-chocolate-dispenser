...
//     var x = [""];
//     for (var i = 0; number > 0; number--, i++) {
//         x[i] = chocolates.shift();
//     }
//     return x;
// }
function removeChocolates(number) {
...
function dispenseChocolates(number) {
    if (number > chocolates.length) {
        return "Insufficient Chocolates in dispencer";
    } else if (number <= 0)
        return "Number cannot be zero/negative";
    else {
        for (var i = 0, x = [""]; number > 0; number--, i++) {
            x[i] = chocolates.pop();
        }
        return x;
    }
...
function dispenseChocolatesOfColor(number, color) {
    if (number > chocolates.length) {
        ...
        for (var i = 0, index = 0, x = [""]; i < number; i++, index++) {
